...
import seakers.vassarheur.attribute.AttributeBuilder;
import seakers.vassarheur.attribute.EOAttribute;
import seakers.vassarheur.jessUserFunction.Improve;
import seakers.vassarheur.jessUserFunction.SameOrBetter;
import seakers.vassarheur.jessUserFunction.Worsen;
import org.apache.commons.lang3.StringUtils;
import seakers.vassarheur.spacecraft.LaunchVehicle;
import seakers.vassarheur.template.classes.SlotInfo;
import seakers.vassarheur.template.functions.JessExtension;
...
public class JessInitializer {
    ...
    public void initializeJess(BaseParams params, Rete r, QueryBuilder qb, MatlabFunctions m) {
        try {
            ...
            r.eval("(import seakers.vassarheur.*)");
...
//                payload_coeffs.put("LEO-ISS", payload_ISS);
            ...
            // Load design heuristics computation rules
            loadDesignHeuristicsRules(r, params.designHeuristicsRulesClp);

            // Load constellation related rules
            loadConstellationRules(r, params.constellationRulesClp);
    ...
    private void loadTemplates(Rete r, Workbook xls, String clp) {
        ...
        loadSimpleTemplate(r, xls, "Satellite","MANIFEST::Satellite");
        loadSimpleTemplate(r, xls, "Orbit","DATABASE::Orbit");
        loadSimpleTemplate(r, xls, "Launch-vehicle","DATABASE::Launch-vehicle");
        loadSimpleTemplate(r, xls, "Constellation", "MANIFEST::Constellation");
    ...
    private void loadDesignHeuristicsRules(Rete r, String clp) {
        try {
            r.batch(clp);
        }
        catch (Exception e) {
            System.out.println("EXC in loadDesignHeuristicsRules " + e.getMessage());
            e.printStackTrace();
        }
    }

    private void loadConstellationRules(Rete r, String clp) {
        try {
            r.batch(clp);
        }
        catch (Exception e) {
            System.out.println("EXC in loadConstellationRules " + e.getMessage());
            e.printStackTrace();
        }
    }
