...
public class MOEARadioCombProgressListener implements ProgressListener {
    ...
    public void progressUpdate(ProgressEvent event) {
        ...
        try {
            fileOut = new PrintStream("./src/test/output/radiocomb/0328_running_population"+event.getCurrentNFE()+".txt");
        ...
        if (xd != null) {
            ...
            for (Solution sol : currentPop) {
                System.out.println(EncodingUtils.getInt(sol.getVariable(0))+","+sol.getVariable(1)+","+sol.getVariable(2)+","+sol.getVariable(3)+","+sol.getVariable(4)+","+sol.getVariable(5));
            }
            try {
                PopulationIO.writeObjectives(new File("./src/test/output/radiocomb/0328_objectives"+event.getCurrentNFE()+".txt"), currentPop);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
