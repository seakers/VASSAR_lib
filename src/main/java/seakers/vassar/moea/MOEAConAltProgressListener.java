...
public class MOEAConAltProgressListener implements ProgressListener {
    ...
    public void progressUpdate(ProgressEvent event) {
        ...
        try {
            fileOut = new PrintStream("./src/test/output/conalt/0321_running_population"+event.getCurrentNFE()+".txt");
        ...
        if (xd != null) {
            ...
            for (Solution sol : currentPop) {
                System.out.println(EncodingUtils.getInt(sol.getVariable(0))+","+sol.getVariable(1)+","+EncodingUtils.getInt(sol.getVariable(2)));
            }
            try {
                PopulationIO.writeObjectives(new File("./src/test/output/conalt/0321_objectives"+event.getCurrentNFE()+".txt"), currentPop);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
