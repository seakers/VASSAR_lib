...
public class MOEAConAltProgressListener implements ProgressListener {
    ...
    public void progressUpdate(ProgressEvent event) {
        ...
        try {
            fileOut = new PrintStream(filepath+"0707_running_population"+event.getCurrentNFE()+".txt");
        ...
        if (xd != null) {
            ...
            for (Solution sol : currentPop) {
                System.out.println(EncodingUtils.getInt(sol.getVariable(0))+","+sol.getVariable(1)+","+EncodingUtils.getInt(sol.getVariable(2))+","+EncodingUtils.getInt(sol.getVariable(3)));
            }
            try {
                PopulationIO.writeObjectives(new File(filepath+"0707_objectives"+event.getCurrentNFE()+".txt"), currentPop);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
