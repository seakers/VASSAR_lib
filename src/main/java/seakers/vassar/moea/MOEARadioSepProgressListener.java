...
public class MOEARadioSepProgressListener implements ProgressListener {
    ...
    public void progressUpdate(ProgressEvent event) {
        ...
        try {
            fileOut = new PrintStream("./src/test/output/radiosep/0324_running_population"+event.getCurrentNFE()+".txt");
        ...
        if (xd != null) {
            ...
            for (Solution sol : currentPop) {
                System.out.println(sol.getVariable(0)+","+EncodingUtils.getInt(sol.getVariable(1))+","+EncodingUtils.getInt(sol.getVariable(2))+","+EncodingUtils.getInt(sol.getVariable(3)));
            }
            try {
                PopulationIO.writeObjectives(new File("./src/test/output/radiosep/0324_objectives"+event.getCurrentNFE()+".txt"), currentPop);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
