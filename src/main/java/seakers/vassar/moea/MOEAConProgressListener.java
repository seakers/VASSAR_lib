...
public class MOEAConProgressListener implements ProgressListener {
    ...
    public void progressUpdate(ProgressEvent event) {
        ...
        try {
            fileOut = new PrintStream(filepath+"0708_running_population"+event.getCurrentNFE()+".txt");
        ...
        if (xd != null) {
            ...
            try {
                PopulationIO.writeObjectives(new File(filepath+"0708_objectives"+event.getCurrentNFE()+".txt"), currentPop);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
