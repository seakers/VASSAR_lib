...
public class MOEARadioReflProgressListener implements ProgressListener {
    ...
    public void progressUpdate(ProgressEvent event) {
        ...
        try {
            fileOut = new PrintStream("./src/test/output/radiorefl/0426_running_population"+event.getCurrentNFE()+".txt");
        ...
        if (xd != null) {
            ...
            for (Solution sol : currentPop) {
                System.out.println(sol.getVariable(0)+","+sol.getVariable(1)+","+EncodingUtils.getInt(sol.getVariable(2))+","+EncodingUtils.getInt(sol.getVariable(3))+","+sol.getVariable(4)+","+sol.getVariable(5));
            }
            try {
                PopulationIO.writeObjectives(new File("./src/test/output/radiorefl/0426_objectives"+event.getCurrentNFE()+".txt"), currentPop);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
