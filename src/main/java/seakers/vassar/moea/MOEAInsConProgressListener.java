...
public class MOEAInsConProgressListener implements ProgressListener {
    ...
    public void progressUpdate(ProgressEvent event) {
        ...
        try {
            fileOut = new PrintStream(filepath+"0704_running_population"+event.getCurrentNFE()+".txt");
        ...
        if (xd != null) {
            ...
            try {
                PopulationIO.writeObjectives(new File(filepath+"0704_objectives"+event.getCurrentNFE()+".txt"), currentPop);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
