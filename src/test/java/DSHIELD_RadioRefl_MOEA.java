...
public class DSHIELD_RadioRefl_MOEA {
    public static void main(String[] args){
        try{
            ...
            MOEARadioReflProgressListener progressListener = new MOEARadioReflProgressListener();
            NondominatedPopulation result = new Executor().withProblemClass(RadioReflProblem.class).withAlgorithm("NSGA-II").withProperties(properties).distributeOnAllCores().withProgressListener(progressListener).run();
            ...
            PopulationIO.writeObjectives(new File("./src/test/output/radiorefl/objectives.txt"), result);
            OrekitConfig.end();
        } catch (Exception e) {
            e.printStackTrace();
