...
public class RadarInsAlt {
    public static void main(String[] args){
        try{
            OrekitConfig.init(4);
            RadarInsAltProblem problem = new RadarInsAltProblem();
...
//                    initialization),stateFile,10);
            ...
            while (algorithm.getNumberOfEvaluations() < 10000) {
                ...
                try {
                    FileOutputStream f = new FileOutputStream("./src/test/output/radar_insalt/1206_current_population.txt");
                ...
                if(algorithm.getNumberOfEvaluations() % 100 == 0) {
                    try{
                        PrintWriter out = new PrintWriter("./src/test/output/radar_insalt/1206_variables_"+algorithm.getNumberOfEvaluations()+".txt");
                        for (Solution sol : currentPop) {
                    ...
                    try {
                        PopulationIO.writeObjectives(new File("./src/test/output/radar_insalt/1206_objectives_"+algorithm.getNumberOfEvaluations()+".txt"), currentPop);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
