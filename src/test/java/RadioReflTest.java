...
public class RadioReflTest {
    public static void main(String[] args){
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("VASSAR_lib/src/test/java/repeat_orbits_nonSSO_varinc.csv"))) { // CHANGE THIS FOR YOUR IMPLEMENTATION
            String line;
        ...
        for(int i=0; i < 20; i++) {                   // Original value of 20
            for(int j = 1; j <= 4; j++) {           // Number of planes.     Original value of 4
                for(int k = 1; k <= 8; k++) {       // Satellites per plane. Original value of 4
                        ...
                        if (l == 0) {
                            ...
                            for(int m = 0; m < j; m++) {
                                for(int n = 0; n < k; n++) {
                                    ...
                                    OrbitInstrumentObject satellite = new OrbitInstrumentObject(new String[]{"P-band_Reflectometer","L-band_Reflectometer","Aquarius"},orbitName);
                            ...
                            for(int m = 0; m < j; m++) {
                                for(int n = 0; n < k; n++) {
                                    ...
                                    OrbitInstrumentObject satellite = new OrbitInstrumentObject(new String[]{"P-band_Reflectometer","L-band_Reflectometer","FMPL-2"},orbitName);
                                    satellites.add(satellite);
                                }
                            }
                            SimpleArchitecture architecture = new SimpleArchitecture(satellites);
        ...
        for(SimpleArchitecture architecture : architectures) {
            ...
            arch.put("combReflRevisit",architecture.getCombReflRevisit());
            arch.put("combReflCoverage",architecture.getCombReflCoverage());
            arch.put("reflRevisitP",architecture.getReflRevisitP());
            arch.put("reflCoverageP",architecture.getReflCoverageP());
            arch.put("reflRevisitL",architecture.getReflRevisitL());
            arch.put("reflCoverageL",architecture.getReflCoverageL());
            arch.put("radioRevisit",architecture.getRadioRevisit());
            ...
            try{
                FileWriter writer = new FileWriter("reflradoutput.json"); // may want to change this!
