...
public class RadarFixedAlt {
    public static void main(String[] args){
        try{
            OrekitConfig.init(4);
            RadarFixedAltProblem problem = new RadarFixedAltProblem();
...
//                    initialization),stateFile,10);
            ...
            while (algorithm.getNumberOfEvaluations() < 10000) {
                ...
                try {
                    FileOutputStream f = new FileOutputStream("./src/test/output/radar_fixedalt/1206_current_population.txt");
                ...
                if(algorithm.getNumberOfEvaluations() % 100 == 0) {
                    try{
                        PrintWriter out = new PrintWriter("./src/test/output/radar_fixedalt/1206_variables_"+algorithm.getNumberOfEvaluations()+".txt");
                        for (Solution sol : currentPop) {
                            out.println(EncodingUtils.getInt(sol.getVariable(0))+","+EncodingUtils.getInt(sol.getVariable(1))+","+sol.getVariable(2)+","+sol.getVariable(3)+","+sol.getVariable(4)+","+sol.getVariable(5)+","+sol.getVariable(6));
                        }
                        out.close();
                    } catch (FileNotFoundException e) {
                        System.out.println("File not found");
                    ...
                    try {
                        PopulationIO.writeObjectives(new File("./src/test/output/radar_fixedalt/1206_objectives_"+algorithm.getNumberOfEvaluations()+".txt"), currentPop);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
