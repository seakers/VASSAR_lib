function [score_vec,panel_scores_mat,data_continuity_score_vec,lists,cost_vec,subobjective_scores,dc_matrices,orbits] = RBES_Evaluate_MissionSet(mission_set)
...
for i = 1:length(mission_set)
    ...
    if isempty(mission.instrument_list)
        continue;
    end
    if params.ESTIMATE_SCIENCE == 1
        if params.DATA_CONTINUITY == 1 
            [score_vec(i),panel_scores_mat(:,i),~,subobjective_scores{i},data_continuity_score_vec(i),dc_matrices{i},cost_vec(i),orbits{i}] = RBES_Evaluate_Mission(mission);
        else
        ...
        [~,~,~,~,~,~,cost_vec(i),orbits{i}] = RBES_Evaluate_Mission(mission);
    end
