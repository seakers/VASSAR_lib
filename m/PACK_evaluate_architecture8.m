function results = PACK_evaluate_architecture8(arch)
%% PACK_evaluate_architecture8.m
global params
...
%     mission_set{s} = create_test_mission(sat_name,sat_instrs,lds(s),params.lifetime,[]);  
    mission_set{s} = create_test_mission(sat_name,sat_instrs,params.startdate,params.lifetime,[]);  
end

%% Eval costs and best orbits
% params.ESTIMATE_SCIENCE = 1;
% [~,~,~,~,cost_vec,~,~,orbits] = RBES_Evaluate_MissionSet(mission_set);
[~,~,~,~,cost_vec,~,~,orbits,lv_pack_factors] = RBES_Evaluate_MissionSet2(mission_set);
results.cost = sum(cost_vec);
results.mission_costs = cost_vec;
results.lv_pack_factors = lv_pack_factors;
% params.ESTIMATE_SCIENCE = 1;
...
for i = 1:ntrains
    ...
    orbit_str = get_orbit_struct_from_string(unique_orbits{i});
    train = create_test_mission(sat_name,sat_instrs,params.startdate,params.lifetime,orbit_str,[]);
...
results.programmatic_risk = PACK_compute_programmatic_risk(arch);
results.launch_risk = PACK_compute_launch_risk(arch);
