function results2 = RBES_change_results_struct2(results)
    ...
    if ~iscell(results) && length(results) == 1
        ...
        for j = 1:length(fields)
            if isscalar(results{1}.(fields{j}))&& isnumeric(results{1}.(fields{j}))
                results2.(['arr_' fields{j}]) = zeros(narc,1);
            else
                results2.(['arr_' fields{j}]) = cell(narc,1);
            end
        end
        for i = 1:narc
            for j = 1:length(fields)
                if isscalar(results{i}.(fields{j})) && isnumeric(results{i}.(fields{j}))
                    results2.(['arr_' fields{j}])(i) = results{i}.(fields{j});
                else
                    results2.(['arr_' fields{j}]){i} = results{i}.(fields{j});
                end
            end
