...
path = '/Users/nozomihitomi/Dropbox/EOSS/problems/climateCentric/result/AIAA JAIS/analysis/pop_all/';
...
orbName = {'600-polar','600-sso-am','600-sso-dd','800-sso-pm','800-sso-dd'};
maxNumOrb = length(orbName);

%logical indices of instrument assignment
ind_inst = zeros(maxNumInst,maxNumSats * maxNumInst);
for i=1:maxNumInst
    ind_inst(i,(i-1)*(maxNumSats) + 1:i*(maxNumSats)) = true;
end
ind_inst = logical(ind_inst);

% %logical indices of the orbit assignment
% ind_orb = zeros(maxNumSats,maxNumSats * maxNumInst);
% for i=1:maxNumSats
%     ind_orb(i,i*(maxNumInst)) = true;
% end
...
for i=1:maxNumInst
    copyInstPerArch(:,i) = sum(decisions(:,ind_inst(i,:)),2);
end
...
figure(1)
scatter(-objectives(:,1),objectives(:,2),5,nSatsOn)    
colormap jet
colorbar
...
ind = dutyCycle > 1;
dutyCycle(ind) = 1;
meanDC = sum(dutyCycle,2)./nSatsOn;
meanDC(isinf(meanDC)) = 0;
...
subplot(3,2,4)
meanMass = sum(mass,2)./nSatsOn;
meanMass(isinf(meanMass)) = 0;
scatter(-objectives(:,1),objectives(:,2),5,meanMass)
colormap jet
colorbar
title('mean mass')
...
meanPE = sum(packingEfficiency,2)./nSatsOn;
meanPE(isinf(meanPE)) = 0;
scatter(-objectives(:,1),objectives(:,2),5,meanPE)
colormap jet
colorbar
