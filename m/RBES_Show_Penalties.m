function cost_breakdown = RBES_Show_Penalties(varargin)
global params
% nsats = max(arch);
arch = varargin{1};
if nargin >1
...
%         orbit = jess_value(r.eval('?orb'));
        ...
        for ins = 1:ni
            ...
            text(x+dx+dx,y2+4*dy+h2/2,instr_names{ins},'FontSize',8);
            y2 = y2 + 1;
        end
        % Penalties
        y2 = 0;
        ...
        text(x+dx,y2+dy+h2/9,'ADC','FontSize',6);
        text(x+dx,y2+2*dy+h2/9,'MEC','FontSize',6);
        text(x+3*dx,y2+dy+h2/9,'SCA','FontSize',6);
        text(x+3*dx,y2+2*dy+h2/9,'THE','FontSize',6);
        text(x+5*dx,y2+dy+h2/9,'EMC','FontSize',6);
        text(x+5*dx,y2+2*dy+h2/9,'DAT','FontSize',6);
